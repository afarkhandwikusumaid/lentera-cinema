  ...
  if (isEditing) {
    return (
      ...
      <div className="bg-bg-surface p-6 rounded-2xl border border-border">
        <h2 className="text-xl font-bold text-text-primary mb-6">{formData.id ? 'Edit Pengeluaran' : 'Catat Pengeluaran'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input required type="date" className="w-full bg-bg-elevated border border-border rounded-lg p-2.5 text-sm" 
          ...
          <tbody>
            {expenses.map((expense) => (
              <tr key={expense.id} className="border-b border-gray-50 transition-colors">
                <td className="p-4 font-bold text-text-primary text-sm">{expense.description}</td>
                <td className="p-4 text-text-secondary text-sm">{new Date(expense.expense_date).toLocaleDateString('id-ID')}</td>
                <td className="p-4 font-semibold text-red-600 text-sm">
                  {formatCurrency(expense.amount)}
                </td>
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2 transition-opacity">
                    <button onClick={() => handleEdit(expense)} className="p-2 text-text-secondary/70 bg-bg-surface rounded-lg border border-border/50"><Edit2 size={14}/></button>
                    <button onClick={() => handleDelete(expense.id)} className="p-2 text-text-secondary/70 bg-bg-surface rounded-lg border border-border/50"><Trash2 size={14}/></button>
                  </div>
                </td>
              </tr>
            ))}
            {expenses.length === 0 && (
              <tr>
                <td colSpan={4} className="p-8 text-center text-text-secondary/70">Belum ada pengeluaran dicatat.</td>
              </tr>
            )}
          </tbody>
        </table>
